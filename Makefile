...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-strain" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-strain"
endif
